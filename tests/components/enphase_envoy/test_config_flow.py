...
async def test_form_no_inverter_access(hass: HomeAssistant) -> None:
    """Test we disable inverters if the provided user does not have access."""
    await setup.async_setup_component(hass, "persistent_notification", {})
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    ...
    with patch(
        "homeassistant.components.enphase_envoy.config_flow.EnvoyReader.getData",
        side_effect=[
            httpx.HTTPStatusError("any", request=MagicMock(), response=MagicMock()),
            True,
        ],
    ), patch(
        "homeassistant.components.enphase_envoy.async_setup_entry",
